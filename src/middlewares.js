...
exports.checkExpire = async () => {
    setInterval(async () => {
        let expiredUserIds = [];

        let users = await User.find({ admin: { $ne: true } });
        ...
        expiredUserIds.forEach(async expiredId => {
            let expiredUserDocIds = await Company.find({ user_id: expiredId })
            if (expiredUserDocIds === 0) {
                expiredUserIds.forEach(async expiredId => {
                    await User.findOneAndDelete({ _id: expiredId })
                    console.log("user :" + expiredId + " has been deleted")
                })
                return;
            }

            expiredUserDocIds.forEach(async expiredUserDocId => {
                await Company.findOneAndDelete({ _id: expiredUserDocId._id })
                await Balance.findOneAndDelete({ _id: expiredUserDocId._id })
                await ProfitReport.findOneAndDelete({ _id: expiredUserDocId._id })
                await BreakEvenAnalysis.findOneAndDelete({ _id: expiredUserDocId._id })
                console.log("Documents with id :" + expiredUserDocId._id + " has been deleted")
            })
            expiredUserIds.forEach(async expiredId => {
                await User.findOneAndDelete({ _id: expiredId })
                console.log("user :" + expiredId + " has been deleted")
